...
async function main() {
  ...
  async function getCheckoutPlanPrice(plan, period) {
  ...
  async function resolveStripePlanFromPriceId(priceId) {
  ...
  async function getActiveStripePrice(plan, period) {
    return await db.get(`
  ...
  async function getReusableStripeProductId(plan) {
    const row = await db.get(`
  ...
  async function syncStripePricingFromPlansValue(rawValue) {
    ...
    for (const def of defs) {
      ...
      const saveMapping = await db.transaction(async (tx) => {
        await tx.run(`
      ...
      `, def.plan, def.period, def.amountCents, def.currency, productId, price.id);
      });
      await saveMapping();
  ...
  await db.exec(`CREATE TABLE IF NOT EXISTS db_migrations (name TEXT PRIMARY KEY, applied_at TEXT NOT NULL DEFAULT (datetime('now')))`);
  async function hasMigration(name) {return !!(await db.get('SELECT 1 FROM db_migrations WHERE name = ?', name));}
  async function markMigration(name) {await db.run("INSERT OR IGNORE INTO db_migrations (name) VALUES (?)", name);}
...
`);
  ...
  async function ensureProductColumn(name, definition) {
    const cols = await db.columns('leaflet_products');
    if (!cols.includes(name)) await db.exec(`ALTER TABLE leaflet_products ADD COLUMN ${name} ${definition}`);
  }
  await ensureProductColumn('origin_lan1_iso', "TEXT NOT NULL DEFAULT ''");
  await ensureProductColumn('origin_lan2_iso', "TEXT NOT NULL DEFAULT ''");
  await ensureProductColumn('product_image_source', "TEXT NOT NULL DEFAULT ''");
  await ensureProductColumn('product_image_license', "TEXT NOT NULL DEFAULT ''");

  /* ── Migrate: add layout_json to leaflets if missing ── */
  const leafletCols = await db.columns('leaflets');
  if (!leafletCols.includes('layout_json')) {
    await db.exec("ALTER TABLE leaflets ADD COLUMN layout_json TEXT");
  }
  if (!leafletCols.includes('thumbnail')) {
    await db.exec("ALTER TABLE leaflets ADD COLUMN thumbnail TEXT");
  }

  /* ── Migrate: enable origin flags on all existing leaflets (one-time) ── */
  if (!(await hasMigration('origin_flag_default_v1'))) {
    try {
      await db.exec(`
      UPDATE leaflets
    ...
    `);
      await markMigration('origin_flag_default_v1');
...
`);
  const cardTemplateCols = await db.columns('card_layout_templates');
  if (!cardTemplateCols.includes('is_platform')) {
    await db.exec("ALTER TABLE card_layout_templates ADD COLUMN is_platform INTEGER NOT NULL DEFAULT 0");
  }
  const pdfExportCols = await db.columns('leaflet_pdf_exports');
  if (!pdfExportCols.includes('share_token')) {
    await db.exec("ALTER TABLE leaflet_pdf_exports ADD COLUMN share_token TEXT NOT NULL DEFAULT ''");
  }
  if (!pdfExportCols.includes('allow_edit')) {
    await db.exec("ALTER TABLE leaflet_pdf_exports ADD COLUMN allow_edit INTEGER NOT NULL DEFAULT 0");
  }
  await db.exec(`
...
`);
  ...
  if (!(await hasMigration('platform_default_card_templates_v1'))) {
    const normalizeDefaultTemplateNames = await db.transaction(async (tx) => {
      for (const name of PLATFORM_DEFAULT_TEMPLATE_NAMES) {
        await tx.run('UPDATE card_layout_templates SET name = ? WHERE lower(name) = ?', name, name.toLowerCase());
      }
    });
    await normalizeDefaultTemplateNames();
    await markMigration('platform_default_card_templates_v1');
  }

  /* ── Migrate: subscription fields ── */
  const userCols = await db.columns('users');
  if (!userCols.includes('stripe_customer_id')) await db.exec("ALTER TABLE users ADD COLUMN stripe_customer_id TEXT");
  if (!userCols.includes('subscription_plan')) await db.exec("ALTER TABLE users ADD COLUMN subscription_plan TEXT NOT NULL DEFAULT 'free'");
  if (!userCols.includes('subscription_status')) await db.exec("ALTER TABLE users ADD COLUMN subscription_status TEXT NOT NULL DEFAULT 'active'");
  if (!userCols.includes('subscription_period')) await db.exec("ALTER TABLE users ADD COLUMN subscription_period TEXT NOT NULL DEFAULT 'monthly'");
  if (!userCols.includes('subscription_start')) await db.exec("ALTER TABLE users ADD COLUMN subscription_start TEXT");
  if (!userCols.includes('subscription_end')) await db.exec("ALTER TABLE users ADD COLUMN subscription_end TEXT");
  if (!userCols.includes('subscription_email_key')) await db.exec("ALTER TABLE users ADD COLUMN subscription_email_key TEXT");
  if (!userCols.includes('role')) await db.exec("ALTER TABLE users ADD COLUMN role TEXT NOT NULL DEFAULT 'user'");
  await db.run(`
  UPDATE users
...
`);
  await db.run("UPDATE users SET role = CASE WHEN lower(email) = lower(?) THEN 'admin' ELSE 'user' END", PRIMARY_ADMIN_EMAIL);

  // one-time free exports
  try {await db.run("ALTER TABLE users ADD COLUMN free_pdf_used INTEGER DEFAULT 0");} catch (e) {}
  try {await db.run("ALTER TABLE users ADD COLUMN free_book_used INTEGER DEFAULT 0");} catch (e) {}

  // default leaflet
  try {await db.run("ALTER TABLE users ADD COLUMN default_leaflet_id INTEGER DEFAULT NULL");} catch (e) {}
...
`);
  ...
  for (const r of seoDefaults) {
    await db.run(`INSERT OR IGNORE INTO seo_pages (page_key,page_name,page_path,title,description,keywords,og_title,og_description,og_image,canonical_url,robots)
      VALUES (?, ?, ?, ?, ?, '', '', '', '', '', 'index, follow')`, r.key, r.name, r.path, r.title, r.description);
  }
  await db.exec(`
...
`);
  ...
  async function stripeSecretKeyValue() {
    const row = await db.get("SELECT value FROM site_settings WHERE key = 'stripe_secret_key'");
    return String(row?.value || '').trim() || STRIPE_SECRET_KEY;
  }

  async function refreshStripeClient() {
    const secretKey = await stripeSecretKeyValue();
...
`);
  ...
  for (const [page, section, field, value] of pcDefaults) {
    await db.run(`INSERT OR IGNORE INTO page_content (page,section,field,value) VALUES (?,?,?,?)`, page, section, field, value);
  }
  ...
  app.post('/api/stripe/webhook', webhookLimiter, express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    switch (event.type) {
      ...
      case 'checkout.session.completed':{
          ...
          WHERE id = ?
        `, customerId || null, plan, period || 'monthly', parseInt(userId, 10));
      ...
      case 'customer.subscription.updated':{
        ...
        `, resolved.plan, status, resolved.period, startDate, endDate, customerId);
            const paidUser = await db.get('SELECT id, subscription_start FROM users WHERE stripe_customer_id = ?', customerId);
            if (paidUser) {
      ...
      case 'customer.subscription.paused':{
        ...
        SET subscription_plan   = 'free',
            ...
            subscription_end    = ?
        WHERE stripe_customer_id = ?
      `, status, toIso(sub.current_period_end), customerId);
  ...
  async function sendSubscriptionDetailsEmail(userId, details) {
    ...
    const user = await db.get('SELECT id,name,email,subscription_email_key FROM users WHERE id = ?', userId);
    ...
    await db.run('UPDATE users SET subscription_email_key = ? WHERE id = ?', emailKey, userId);
  ...
  function adminMiddleware(req, res, next) {
    authMiddleware(req, res, async () => {
      const row = await db.get('SELECT email, role FROM users WHERE id=?', req.user.id);
      ...
      if (row.role !== 'admin') {
        await db.run("UPDATE users SET role='admin' WHERE id=?", req.user.id);
      }
      next();
    });
  }
  ...
  async function createBackup() {
    ...
    let backupFile;
    if (db.isPostgres) {
      backupFile = path.join(BACKUPS_DIR, `backup-${timestamp}.json.gz`);
      const tables = [
        'users', 'leaflets', 'leaflet_products', 'product_clicks', 'stripe_plan_prices',
        'leaflet_pdf_exports', 'card_layout_templates', 'help_article_groups', 'help_articles',
        'icon_library', 'icon_preset_overrides', 'seo_pages', 'site_settings', 'page_content',
        'db_migrations'
      ];
      const snapshot = {};
      ...
      await new Promise((resolve, reject) => {
        zlib.gzip(JSON.stringify({ created_at: new Date().toISOString(), tables: snapshot }), { level: zlib.constants.Z_BEST_SPEED }, (err, data) => {
          if (err) return reject(err);
          fs.writeFile(backupFile, data, (writeErr) => writeErr ? reject(writeErr) : resolve());
        });
      });
    } else {
      const tempDb = path.join(BACKUPS_DIR, `_tmp_${timestamp}.db`);
      backupFile = path.join(BACKUPS_DIR, `backup-${timestamp}.db.gz`);
    ...
    const maxKeep = parseInt((await getSetting('backup_max_keep')) || '20', 10);
  ...
  async function startAutoBackup() {
    ...
    const enabled = await getSetting('backup_auto_enabled');
    const hours = parseFloat((await getSetting('backup_auto_hours')) || '24');
  ...
  await db.run("INSERT OR IGNORE INTO site_settings (key,value) VALUES ('backup_auto_enabled','0')");
  await db.run("INSERT OR IGNORE INTO site_settings (key,value) VALUES ('backup_auto_hours','24')");
  await db.run("INSERT OR IGNORE INTO site_settings (key,value) VALUES ('backup_max_keep','20')");
  ...
  const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
      if (/^image\/(jpeg|jpg|png|webp|gif|avif|svg\+xml)$/i.test(file.mimetype)) cb(null, true);else
      cb(new Error('Only image files are allowed (jpg, png, svg, webp, gif, avif).'));
    }
  });
  const pdfExportUpload = multer({
    ...
    fileFilter: (req, file, cb) => {
      if (file.mimetype === 'application/pdf' || /\.pdf$/i.test(file.originalname || '')) cb(null, true);else
  ...
  app.post('/api/signup', authLimiter, async (req, res, next) => {
    try {
      ...
      if (!email) errors.email = 'Email is required.';else
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) errors.email = 'Please enter a valid email.';else
      if (email.length > 190) errors.email = 'Email is too long.';
      ...
      VALUES (?, ?, ?, 0, ?, ?)
    `, name, email, passwordHash, verifyToken, verifyExpires);
  ...
  app.get('/api/oauth/google/callback', authLimiter, async (req, res, next) => {
    try {
      ...
      let user = await db.get('SELECT id, name, email FROM users WHERE email = ?', email);
      if (user) {
        await db.run('UPDATE users SET email_verified = 1, verify_token = NULL, verify_token_expires = NULL WHERE id = ?', user.id);
      } else {
  ...
  app.get('/api/user/default-leaflet', authMiddleware, async (req, res, next) => {
    try {
      const user = await db.get('SELECT default_leaflet_id FROM users WHERE id = ?', req.user.id);
      if (!user) {res.status(404).json({ error: 'User not found' });return;}
      const leafletId = user.default_leaflet_id;
      if (!leafletId) {res.json({ default_leaflet_id: null, default_leaflet: null });return;}
      const leaflet = await db.get('SELECT id, title, layout_json FROM leaflets WHERE id = ? AND user_id = ?', leafletId, req.user.id);
      if (!leaflet) {
        // default was deleted — clear it
        await db.run('UPDATE users SET default_leaflet_id = NULL WHERE id = ?', req.user.id);
  ...
  app.put('/api/user/default-leaflet', authMiddleware, async (req, res, next) => {
    try {
      ...
      const leaflet = await db.get('SELECT id FROM leaflets WHERE id = ? AND user_id = ?', leaflet_id, req.user.id);
      if (!leaflet) {res.status(404).json({ error: 'Leaflet not found.' });return;}
      await db.run('UPDATE users SET default_leaflet_id = ? WHERE id = ?', leaflet_id, req.user.id);
  ...
  app.post('/api/leaflets/:id/duplicate', authMiddleware, async (req, res, next) => {
    try {
      const src = await db.get('SELECT * FROM leaflets WHERE id = ? AND user_id = ?', req.params.id, req.user.id);
      if (!src) {res.status(404).json({ error: 'Leaflet not found.' });return;}

      const newTitle = (req.body?.title || '').trim() || `${src.title} (Copy)`;
      const insertLeaflet = await db.run(
        `INSERT INTO leaflets (user_id, title, description, language_mode, layout_json) VALUES (?, ?, ?, ?, ?)`,
        req.user.id,
        newTitle,
        src.description || '',
        src.language_mode || 'one',
        src.layout_json || null
      );
      const newId = insertLeaflet.lastInsertRowid;

      // Copy products
      const srcProducts = await db.all('SELECT * FROM leaflet_products WHERE leaflet_id = ? ORDER BY row_index', req.params.id);
      const copyAll = await db.transaction(async (tx, rows) => {
        for (const p of rows) {
          await tx.run(`INSERT INTO leaflet_products
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            newId, p.row_index, p.product_name_lan1, p.product_name_lan2,
  ...
  /* ── POST /api/leaflets ── */
  async function defaultLayoutForNewLeaflet() {
    const setting = await db.get("SELECT value FROM site_settings WHERE key = 'default_card_template_id'");
  ...
  async function defaultCardTemplateRows() {
    let fallbackLayouts = {};
    try {
    ...
    return Promise.all(PLATFORM_DEFAULT_TEMPLATE_NAMES.map(async (name, index) => {
      const row = await db.get(
  ...
  app.post('/api/leaflets', authMiddleware, async (req, res, next) => {
    try {
      ...
      const leafletResult = await db.run(
        `INSERT INTO leaflets (user_id, title, description, language_mode, layout_json) VALUES (?, ?, ?, ?, ?)`,
        req.user.id, String(title).trim(), String(description).trim(),
        languageMode === 'two' ? 'two' : 'one',
        JSON.stringify(await defaultLayoutForNewLeaflet())
      );
      const leafletId = leafletResult.lastInsertRowid;

      const insertAll = await db.transaction(async (tx, rows) => {
        for (const p of rows) {
          await tx.run(`
      ...
      if (Array.isArray(products) && products.length > 0) await insertAll(products);

      const leaflet = await db.get('SELECT * FROM leaflets WHERE id = ?', leafletId);
      res.status(201).json({ id: leaflet.id, title: leaflet.title, createdAt: leaflet.created_at });
    } catch (err) {next(err);}
  });

  /* ── GET /api/leaflets ── */
  app.get('/api/leaflets', authMiddleware, async (req, res, next) => {
    try {
      const leaflets = await db.all(
  ...
  app.put('/api/leaflets/:id/thumbnail', authMiddleware, async (req, res, next) => {
    try {
      const { thumbnail } = req.body;
      if (!thumbnail || typeof thumbnail !== 'string') {
        res.status(400).json({ error: 'thumbnail is required' });return;
      }
      const leaflet = await db.get('SELECT id FROM leaflets WHERE id = ? AND user_id = ?', req.params.id, req.user.id);
      if (!leaflet) {res.status(404).json({ error: 'Leaflet not found.' });return;}
      await db.run('UPDATE leaflets SET thumbnail = ? WHERE id = ?', thumbnail, req.params.id);
  ...
  app.post('/api/leaflets/:id/exported-pdfs', authMiddleware, (req, res) => {
    pdfExportUpload.single('pdf')(req, res, async (err) => {
      try {
        ...
        const leaflet = await db.get('SELECT id, title FROM leaflets WHERE id = ? AND user_id = ?', req.params.id, req.user.id);
        if (!leaflet) {res.status(404).json({ error: 'Leaflet not found.' });return;}
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `, req.user.id, leaflet.id, filename, req.file.originalname || `${safeBase}.pdf`, req.file.size || req.file.buffer.length, shareToken, allowEdit ? 1 : 0);
        res.status(201).json({
          export: {
  ...
  app.get('/api/shared-pdfs/:token', async (req, res) => {
    const row = await db.get(`
  ...
  app.get('/api/shared-pdfs/:token/meta', async (req, res) => {
    const row = await db.get(`
  ...
  app.get('/api/leaflets/:id/exported-pdfs', authMiddleware, async (req, res) => {
    const leaflet = await db.get('SELECT id FROM leaflets WHERE id = ? AND user_id = ?', req.params.id, req.user.id);
    if (!leaflet) {res.status(404).json({ error: 'Leaflet not found.' });return;}
    ...
    ORDER BY created_at DESC
    LIMIT 25
  `, req.user.id, leaflet.id)).map((row) => ({
      ...row,
  ...
  app.get('/api/leaflets/:id/exported-pdfs/:filename', authMiddleware, async (req, res) => {
    const row = await db.get(`
    SELECT e.filename, e.original_name
    FROM leaflet_pdf_exports e
    JOIN leaflets l ON l.id = e.leaflet_id
    WHERE e.leaflet_id = ? AND e.user_id = ? AND l.user_id = ? AND e.filename = ?
  `, req.params.id, req.user.id, req.user.id, req.params.filename);
  ...
  app.delete('/api/leaflets/:id', authMiddleware, async (req, res, next) => {
    try {
      const leaflet = await db.get('SELECT id FROM leaflets WHERE id = ? AND user_id = ?', req.params.id, req.user.id);
      if (!leaflet) {res.status(404).json({ error: 'Leaflet not found.' });return;}
      await db.run('DELETE FROM leaflet_products WHERE leaflet_id = ?', leaflet.id);
      await db.run('DELETE FROM leaflets WHERE id = ?', leaflet.id);
      res.json({ success: true });
    } catch (err) {next(err);}
  });

  /* ── GET /api/leaflets/:id ── */
  app.get('/api/leaflets/:id', authMiddleware, async (req, res, next) => {
    try {
  ...
  app.get('/api/admin/leaflets/:id', adminMiddleware, async (req, res, next) => {
    try {
      const leaflet = await db.get(
  ...
  app.get('/api/admin/leaflets/:id/layout', adminMiddleware, async (req, res, next) => {
    try {
      const leaflet = await db.get('SELECT id, layout_json FROM leaflets WHERE id = ?', req.params.id);
      if (!leaflet) {res.status(404).json({ error: 'Leaflet not found.' });return;}
  ...
  app.post('/api/leaflets/:lid/products', authMiddleware, async (req, res, next) => {
    try {
      const leaflet = await db.get('SELECT id FROM leaflets WHERE id = ? AND user_id = ?', req.params.lid, req.user.id);
      if (!leaflet) {res.status(404).json({ error: 'Leaflet not found.' });return;}
      ...
      const maxRow = await db.get('SELECT COALESCE(MAX(row_index),0) AS m FROM leaflet_products WHERE leaflet_id = ?', leaflet.id);
    ...
    `,
      ...
      const product = await db.get('SELECT * FROM leaflet_products WHERE id = ?', result.lastInsertRowid);
      res.status(201).json({ product });
    } catch (err) {next(err);}
  });

  /* ── PUT /api/leaflets/:lid/products/:pid ── */
  app.put('/api/leaflets/:lid/products/:pid', authMiddleware, async (req, res, next) => {
    try {
      const leaflet = await db.get(
    ...
    `,
      ...
      const updated = await db.get('SELECT * FROM leaflet_products WHERE id = ?', product.id);
      res.json({ product: updated });
    } catch (err) {next(err);}
  });

  /* ── DELETE /api/leaflets/:lid/products/:pid ── */
  app.delete('/api/leaflets/:lid/products/:pid', authMiddleware, async (req, res, next) => {
    try {
      const leaflet = await db.get(
        'SELECT id FROM leaflets WHERE id = ? AND user_id = ?',
        req.params.lid, req.user.id);
      ...
      await db.run('DELETE FROM leaflet_products WHERE id = ?', product.id);
      res.json({ success: true });
    } catch (err) {next(err);}
  });

  /* ── GET /api/leaflets/:id/layout ── */
  app.get('/api/leaflets/:id/layout', authMiddleware, async (req, res, next) => {
    try {
      const leaflet = await db.get('SELECT id, layout_json FROM leaflets WHERE id = ? AND user_id = ?',
      req.params.id, req.user.id);
      if (!leaflet) {res.status(404).json({ error: 'Leaflet not found.' });return;}
      const raw = leaflet.layout_json ? JSON.parse(leaflet.layout_json) : null;
  ...
  app.put('/api/leaflets/:id/layout', authMiddleware, async (req, res, next) => {
    try {
      const leaflet = await db.get('SELECT id FROM leaflets WHERE id = ? AND user_id = ?',
      req.params.id, req.user.id);
      if (!leaflet) {res.status(404).json({ error: 'Leaflet not found.' });return;}
      ...
      await db.run('UPDATE leaflets SET layout_json = ? WHERE id = ?',
      JSON.stringify(layout), leaflet.id);
      res.json({ layout });
    } catch (err) {next(err);}
  });

  /* ── PUT /api/leaflets/:id/layout/reset ── */
  app.put('/api/leaflets/:id/layout/reset', authMiddleware, async (req, res, next) => {
    try {
      const leaflet = await db.get('SELECT id FROM leaflets WHERE id = ? AND user_id = ?',
      req.params.id, req.user.id);
      if (!leaflet) {res.status(404).json({ error: 'Leaflet not found.' });return;}
      await db.run('UPDATE leaflets SET layout_json = NULL WHERE id = ?', leaflet.id);
  ...
  app.get('/api/layout-templates', authMiddleware, async (req, res) => {
    const defaultRows = await defaultCardTemplateRows();
    ...
    const userRows = await db.all(
     ...
     WHERE t.user_id = ?
       ...
       AND lower(t.name) NOT IN (${PLATFORM_DEFAULT_TEMPLATE_NAMES.map(() => 'lower(?)').join(',')})
     ORDER BY t.created_at DESC`,
      req.user.id, ...PLATFORM_DEFAULT_TEMPLATE_NAMES);
  ...
  app.post('/api/layout-templates', authMiddleware, async (req, res) => {
    ...
    const result = await db.run(
      ...
      req.user.id, name, layoutJson, isPlatform ? 1 : 0);
    res.status(201).json({
      template: {
        id: result.lastInsertRowid,
        name,
  ...
  app.get('/api/admin/card-templates', adminMiddleware, async (req, res) => {
    const platformRows = await db.all(
    ...
    const rows = [
    ...(await defaultCardTemplateRows()).map((row) => ({ ...row, is_platform: 1 })),
  ...
  app.post('/api/admin/card-templates', adminMiddleware, async (req, res) => {
  ...
  app.put('/api/admin/card-templates/default', adminMiddleware, async (req, res) => {
    const id = parseInt(String(req.body.template_id || ''), 10);
    if (Number.isNaN(id) || id <= 0) {
      res.status(400).json({ error: 'Valid template_id is required.' });
      return;
    }
    const row = await db.get('SELECT id, name FROM card_layout_templates WHERE id = ?', id);
    if (!row) {
      res.status(404).json({ error: 'Template not found.' });
      return;
    }
    await db.run('INSERT OR REPLACE INTO site_settings (key, value) VALUES (?, ?)', 'default_card_template_id', String(id));
    res.json({ ok: true, template_id: id, template_name: row.name });
  });

  app.put('/api/layout-templates/:id', authMiddleware, async (req, res) => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) {res.status(400).json({ error: 'Invalid template id.' });return;}
    const row = await db.get('SELECT id, user_id, name, layout_json, is_platform, created_at FROM card_layout_templates WHERE id = ?', id);
    ...
    await db.run('UPDATE card_layout_templates SET name = ?, layout_json = ?, is_platform = ? WHERE id = ?',
    name, JSON.stringify(layout), requestedPlatform ? 1 : 0, id);
    res.json({
      template: {
        id,
        name,
  ...
  app.delete('/api/layout-templates/:id', authMiddleware, async (req, res) => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) {res.status(400).json({ error: 'Invalid template id.' });return;}
    const row = await db.get('SELECT id, user_id FROM card_layout_templates WHERE id = ?', id);
    if (!row) {res.status(404).json({ error: 'Template not found.' });return;}
    const defaultRow = await db.get(`SELECT id FROM card_layout_templates WHERE id = ? AND name IN (${PLATFORM_DEFAULT_TEMPLATE_NAMES.map(() => '?').join(',')})`, id, ...PLATFORM_DEFAULT_TEMPLATE_NAMES);
    if (defaultRow) {
    ...
    await db.run('DELETE FROM card_layout_templates WHERE id = ?', id);
    res.json({ success: true });
  });
  ...
  app.post('/api/track/click', async (req, res) => {
    ...
    await db.run(
      ...
      Number(product_id), Number(leaflet_id), Number(user_id) || 0);
    res.json({ ok: true });
  });

  /* ── GET /api/user/insights ── */
  app.get('/api/user/insights', authMiddleware, async (req, res) => {
    const uid = req.user.id;

    /* 1. All leaflets with created_at */
    const allLeaflets = await db.all(
  ...
  `, uid);
    ...
    const weeklyClicks = await db.all(`
  ...
  `, req.user.id);
    const user = await db.get(
      ...
      req.user.id);
    const firstLeaflet = await db.get(`
  ...
  app.delete('/api/user/account', authMiddleware, async (req, res) => {
    ...
    await db.run('DELETE FROM leaflet_products WHERE leaflet_id IN (SELECT id FROM leaflets WHERE user_id = ?)', req.user.id);
    await db.run('DELETE FROM leaflets WHERE user_id = ?', req.user.id);
    await db.run('DELETE FROM users WHERE id = ?', req.user.id);
    res.json({ message: 'Account deleted.' });
  });

  /* ── GET /api/user/export-quota  — returns free export usage for current user ── */
  app.get('/api/user/export-quota', authMiddleware, async (req, res) => {
    const user = await db.get('SELECT email, role, subscription_plan, free_pdf_used, free_book_used FROM users WHERE id = ?', req.user.id);
    if (!user) return res.status(404).json({ error: 'User not found' });
    const pdfLimitRow = await db.get("SELECT value FROM site_settings WHERE key = 'free_pdf_export_limit'");
  ...
  app.post('/api/user/consume-export', authMiddleware, async (req, res) => {
    ...
    Math.max(0, Math.min(10000, Number.parseInt(
      (await db.get("SELECT value FROM site_settings WHERE key = 'free_pdf_export_limit'"))?.value || '1',
      10
    ) || 0)) :
    1;
    if (current >= limit) return res.status(403).json({ error: 'Free export limit reached', limitReached: true, limit });
    await db.run(`UPDATE users SET ${col} = ${col} + 1 WHERE id = ?`, req.user.id);
  ...
  app.post('/api/stripe/confirm-session', paymentLimiter, authMiddleware, async (req, res) => {
    ...
    `,
      ...
      const updatedUser = await db.get('SELECT subscription_start FROM users WHERE id = ?', req.user.id);
  ...
  async function getOrCreateStripeCustomer(user) {
    ...
    if (customerId) {
      ...
      await db.run('UPDATE users SET stripe_customer_id = NULL WHERE id = ?', user.id);
    }
  ...
  app.post('/api/stripe/create-portal-session', paymentLimiter, authMiddleware, async (req, res) => {
    ...
    try {
      ...
      if (isStripeMissingCustomerError(err)) {
        await db.run('UPDATE users SET stripe_customer_id = NULL WHERE id = ?', req.user.id);
  ...
  app.get('/api/admin/stats', adminMiddleware, async (req, res) => {
    ...
    const newUsersWeek = (await db.get("SELECT COUNT(*) as n FROM users WHERE created_at >= datetime('now','-7 days')")).n;
    const totalLeaflets = (await db.get('SELECT COUNT(*) as n FROM leaflets')).n;
    const newLeafletsWeek = (await db.get("SELECT COUNT(*) as n FROM leaflets WHERE created_at >= datetime('now','-7 days')")).n;
    const totalProducts = (await db.get('SELECT COUNT(*) as n FROM leaflet_products')).n;
    const totalClicks = (await db.get('SELECT COUNT(*) as n FROM product_clicks')).n;
    const planBreakdown = await db.all("SELECT subscription_plan, COUNT(*) as n FROM users GROUP BY subscription_plan");
    const recentUsers = await db.all("SELECT id,name,email,role,subscription_plan,created_at FROM users ORDER BY created_at DESC LIMIT 5");
    const recentLeaflets = await db.all(`SELECT l.id,l.title,l.created_at,u.name as owner,u.email,
    (SELECT COUNT(*) FROM leaflet_products WHERE leaflet_id=l.id) as product_count
    FROM leaflets l JOIN users u ON u.id=l.user_id ORDER BY l.created_at DESC LIMIT 5`);
  ...
  `, like, like, Number(limit), off);
    const total = (await db.get(`SELECT COUNT(*) as n FROM users WHERE name LIKE ? OR email LIKE ?`, like, like)).n;
  ...
  app.put('/api/admin/users/:id', adminMiddleware, async (req, res) => {
    ...
    const updated = await db.get('SELECT id,name,email,role,email_verified,subscription_plan,subscription_status,created_at FROM users WHERE id=?', id);
    res.json(updated);
  });
  ...
  app.get('/api/admin/leaflets', adminMiddleware, async (req, res) => {
  ...
  `, like, like, like, Number(limit), off);
    const total = (await db.get(`SELECT COUNT(*) as n FROM leaflets l JOIN users u ON u.id=l.user_id WHERE l.title LIKE ? OR u.name LIKE ? OR u.email LIKE ?`, like, like, like)).n;
    res.json({ leaflets: rows, total, page: Number(page), limit: Number(limit) });
  });

  /* ── DELETE /api/admin/leaflets/:id ── */
  app.delete('/api/admin/leaflets/:id', adminMiddleware, async (req, res) => {
    const id = Number(req.params.id);
    const tx = await db.transaction(async (txDb) => {
      await txDb.run('DELETE FROM leaflet_products WHERE leaflet_id=?', id);
      await txDb.run('DELETE FROM leaflets WHERE id=?', id);
    });
    await tx();
    res.json({ ok: true });
  });
  ...
  app.delete('/api/admin/leaflets', adminMiddleware, async (req, res) => {
    const ids = Array.isArray(req.body?.ids) ? req.body.ids.map(Number).filter(Boolean) : [];
    if (!ids.length) return res.status(400).json({ error: 'No ids provided' });
    const placeholders = ids.map(() => '?').join(',');
    const tx = await db.transaction(async (txDb) => {
      await txDb.run(`DELETE FROM leaflet_products WHERE leaflet_id IN (${placeholders})`, ...ids);
      await txDb.run(`DELETE FROM leaflets WHERE id IN (${placeholders})`, ...ids);
    });
    await tx();
  ...
  app.get('/api/icons', authMiddleware, async (req, res) => {
  ...
  `);
    const preset_overrides = await db.all(`
  ...
  app.get('/api/admin/preset-icons', adminMiddleware, async (req, res) => {
  ...
  app.put('/api/admin/preset-icons/:key', adminMiddleware, async (req, res) => {
  ...
  `, iconKey, label, active, Number.isFinite(sortOrder) ? sortOrder : 0, deleted);
  ...
  app.delete('/api/admin/preset-icons/:key', adminMiddleware, async (req, res) => {
    const iconKey = String(req.params.key || '').trim().slice(0, 120);
    if (!iconKey) {
      res.status(400).json({ error: 'Icon key is required.' });return;
    }
  ...
  app.get('/api/admin/icons', adminMiddleware, async (req, res) => {
    const icons = await db.all(`
    SELECT id, label, url, active, sort_order, created_at
    FROM icon_library
    ORDER BY sort_order ASC, created_at DESC, id DESC
  ...
  app.post('/api/admin/icons', adminMiddleware, upload.single('icon'), async (req, res) => {
    if (!req.file) {
    ...
    VALUES (?, ?, ?, ?)
  `, label || fallbackLabel || 'Icon', url, active, Number.isFinite(sortOrder) ? sortOrder : 0);
    const icon = await db.get('SELECT id, label, url, active, sort_order, created_at FROM icon_library WHERE id=?', result.lastInsertRowid);
    res.json({ icon });
  });

  app.put('/api/admin/icons/:id', adminMiddleware, async (req, res) => {
    const id = Number(req.params.id);
    const existing = await db.get('SELECT * FROM icon_library WHERE id=?', id);
    if (!existing) {
    ...
    await db.run('UPDATE icon_library SET label=?, active=?, sort_order=? WHERE id=?',
    label || existing.label, active, Number.isFinite(sortOrder) ? sortOrder : existing.sort_order, id);
    const icon = await db.get('SELECT id, label, url, active, sort_order, created_at FROM icon_library WHERE id=?', id);
    res.json({ icon });
  });

  app.delete('/api/admin/icons/:id', adminMiddleware, async (req, res) => {
    const id = Number(req.params.id);
    const icon = await db.get('SELECT * FROM icon_library WHERE id=?', id);
    if (!icon) {
      res.status(404).json({ error: 'Icon not found.' });return;
    }
    await db.run('DELETE FROM icon_library WHERE id=?', id);
  ...
  app.delete('/api/admin/deal-tags/:key', adminMiddleware, async (req, res) => {
    ...
    const row = await db.get("SELECT value FROM site_settings WHERE key = 'deleted_deal_tags'");
    let deleted = [];
    try {
  ...
  app.put('/api/admin/settings', adminMiddleware, async (req, res) => {
    const tx = await db.transaction(async (txDb, entries) => {
      for (const [k, v] of entries) {
        await txDb.run('INSERT OR REPLACE INTO site_settings (key,value) VALUES (?,?)', k, String(v));
      }
    });
    await tx(Object.entries(req.body));
  ...
  async function readCoverLayoutTemplates() {
    const row = await db.get("SELECT value FROM site_settings WHERE key = 'cover_layout_templates'");
    try {
  ...
  async function writeCoverLayoutTemplates(templates) {
    await db.run("INSERT OR REPLACE INTO site_settings (key, value) VALUES ('cover_layout_templates', ?)", JSON.stringify(templates));
  }
  ...
  app.get('/api/admin/seo', adminMiddleware, async (req, res) => {
    const rows = await db.all('SELECT * FROM seo_pages ORDER BY page_name ASC');
    res.json(rows);
  });

  /* ── PUT /api/admin/seo/:id ── update one SEO page ── */
  app.put('/api/admin/seo/:id', adminMiddleware, async (req, res) => {
  ...
  app.get('/api/admin/check', async (req, res) => {
    const hasAdmin = !!(await db.get('SELECT id FROM users WHERE lower(email)=lower(?) LIMIT 1', PRIMARY_ADMIN_EMAIL));
    res.json({ hasAdmin });
  });
  ...
  app.post('/api/admin/setup', authMiddleware, async (req, res) => {
    const self = await db.get('SELECT id,email FROM users WHERE id=?', req.user.id);
    ...
    await db.run('UPDATE users SET role=? WHERE lower(email) <> lower(?)', 'user', PRIMARY_ADMIN_EMAIL);
    await db.run("UPDATE users SET role='admin' WHERE id=?", req.user.id);
    const user = await db.get('SELECT id,name,email,role FROM users WHERE id=?', req.user.id);
  ...
  app.get('/api/admin/backup/settings', adminMiddleware, async (req, res) => {
    res.json({
      auto_enabled: (await getSetting('backup_auto_enabled')) || '0',
      auto_hours: (await getSetting('backup_auto_hours')) || '24',
      max_keep: (await getSetting('backup_max_keep')) || '20'
    });
  });

  /* PUT  /api/admin/backup/settings ── save auto-backup config + restart scheduler */
  app.put('/api/admin/backup/settings', adminMiddleware, async (req, res) => {
    const { auto_enabled, auto_hours, max_keep } = req.body;
    if (auto_enabled !== undefined) await db.run('INSERT OR REPLACE INTO site_settings (key,value) VALUES (?,?)', 'backup_auto_enabled', String(auto_enabled));
    if (auto_hours !== undefined) await db.run('INSERT OR REPLACE INTO site_settings (key,value) VALUES (?,?)', 'backup_auto_hours', String(auto_hours));
    if (max_keep !== undefined) await db.run('INSERT OR REPLACE INTO site_settings (key,value) VALUES (?,?)', 'backup_max_keep', String(max_keep));
    await startAutoBackup();
    res.json({ ok: true });
  });
  ...
  app.get('/api/admin/pages/:page', adminMiddleware, async (req, res) => {
    const rows = await db.all('SELECT section,field,value FROM page_content WHERE page=?', req.params.page);
    res.json(buildPageContent(rows));
  });
    ...
    const tx = await db.transaction(async (txDb) => {
      for (const [section, fields] of Object.entries(entries)) {
        ...
        for (const [field, value] of Object.entries(fields)) {
          await txDb.run('INSERT OR REPLACE INTO page_content (page,section,field,value) VALUES (?,?,?,?)', page, section, field, String(value));
        }
      }
    });
    await tx();
  ...
  app.get('/api/help-groups', async (req, res) => {
    const groups = await db.all('SELECT * FROM help_article_groups ORDER BY sort_order ASC, id ASC');
    const articles = await db.all('SELECT * FROM help_articles ORDER BY sort_order ASC, id ASC');
  ...
  app.get('/api/admin/help-groups', adminMiddleware, async (req, res) => {
    const groups = await db.all('SELECT * FROM help_article_groups ORDER BY sort_order ASC, id ASC');
    const articles = await db.all('SELECT * FROM help_articles ORDER BY sort_order ASC, id ASC');
    const result = groups.map((g) => ({
      ...g,
  ...
  app.post('/api/admin/help-groups', adminMiddleware, async (req, res) => {
    const { icon = '📄', label, sort_order = 0 } = req.body;
    if (!label) return res.status(400).json({ error: 'label is required' });
    const r = await db.run('INSERT INTO help_article_groups (icon, label, sort_order) VALUES (?,?,?)', icon, label, sort_order);
    res.json(await db.get('SELECT * FROM help_article_groups WHERE id=?', r.lastInsertRowid));
  });

  /* PUT /api/admin/help-groups/:id */
  app.put('/api/admin/help-groups/:id', adminMiddleware, async (req, res) => {
    const { icon, label, sort_order } = req.body;
    const id = Number(req.params.id);
    const g = await db.get('SELECT * FROM help_article_groups WHERE id=?', id);
    if (!g) return res.status(404).json({ error: 'Not found' });
    await db.run('UPDATE help_article_groups SET icon=?, label=?, sort_order=? WHERE id=?',
    icon ?? g.icon, label ?? g.label, sort_order ?? g.sort_order, id);
    res.json(await db.get('SELECT * FROM help_article_groups WHERE id=?', id));
  });

  /* DELETE /api/admin/help-groups/:id */
  app.delete('/api/admin/help-groups/:id', adminMiddleware, async (req, res) => {
    const id = Number(req.params.id);
    await db.run('DELETE FROM help_articles WHERE group_id=?', id);
    await db.run('DELETE FROM help_article_groups WHERE id=?', id);
    res.json({ ok: true });
  });

  /* POST /api/admin/help-articles */
  app.post('/api/admin/help-articles', adminMiddleware, async (req, res) => {
    ...
    const r = await db.run('INSERT INTO help_articles (group_id, title, desc, content, image_url, sort_order) VALUES (?,?,?,?,?,?)', group_id, title, desc, content, image_url, sort_order);
    res.json(await db.get('SELECT * FROM help_articles WHERE id=?', r.lastInsertRowid));
  });

  /* PUT /api/admin/help-articles/:id */
  app.put('/api/admin/help-articles/:id', adminMiddleware, async (req, res) => {
    const { title, desc, content, image_url, sort_order, group_id } = req.body;
    const id = Number(req.params.id);
    const a = await db.get('SELECT * FROM help_articles WHERE id=?', id);
    if (!a) return res.status(404).json({ error: 'Not found' });
    await db.run('UPDATE help_articles SET group_id=?, title=?, desc=?, content=?, image_url=?, sort_order=? WHERE id=?',
    group_id ?? a.group_id, title ?? a.title, desc ?? a.desc, content ?? a.content, image_url !== undefined ? image_url : a.image_url, sort_order ?? a.sort_order, id);
    res.json(await db.get('SELECT * FROM help_articles WHERE id=?', id));
  });

  /* DELETE /api/admin/help-articles/:id */
  app.delete('/api/admin/help-articles/:id', adminMiddleware, async (req, res) => {
    await db.run('DELETE FROM help_articles WHERE id=?', Number(req.params.id));
    res.json({ ok: true });
  });
