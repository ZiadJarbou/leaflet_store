...
const A4_NANO_TEMPLATE_PROMPTS = [
    'Premium product leaflet cover with bold headline area, product collage, clean price highlights, modern retail design.',
    'Elegant supermarket catalogue cover with fresh food photography, clear brand space, refined typography, bright commercial style.',
    'Luxury promotional flyer cover with dark background, gold accents, hero product area, high-end retail composition.',
    'Modern electronics leaflet cover with dynamic product grid, blue accent lighting, clean offer badges, professional layout.',
];
...
function enhanceA4CoverPrompt(prompt: string, orientation: 'portrait' | 'landscape') {
    ...
    return [
        `${format}. ${prompt.trim()}`,
        'Fast clean leaflet cover, sharp commercial style, clear margins.',
    ].join(' ');
}
...
function LeafletView({ coverBuilderOnly = false, leafletId, nanoA4VisibleOverride }: LeafletViewModeProps) {
    ...
    async function applyGeneratedCoverImage(imageUrl: string) {
        const hasSelectedProducts = coverBuilder.selectedProductIds.length > 0;
        if (!hasSelectedProducts) {
            setCoverPage({ image: imageUrl, show: true, builder: false });
            setCurrentPage(-1);
            return;
        }
        const dynamicCoverBuilder = cloneCoverBuilderState({
            ...
            bgType: 'image',
            bgImage: imageUrl,
            visibleItems: { ...coverBuilder.visibleItems, products: true },
        });
      ...
      <div className="lv-layout">
        ...
        <aside className={`lv-sidebar${openSbSection ? ' lv-sidebar--open' : ''}`}>
          ...
          <SbSection id="firstpage" open={openSbSection === 'firstpage'} onToggle={() => { }} title="Cover Page" tooltip={SB_TOOLTIPS.firstpage}>
            ...
            {nanoA4Enabled && (<div className="lv-nano-a4">
              <div className="lv-nano-head">
                <div>
                  <span>Nanobanana A4</span>
                  <strong>Cover generator</strong>
                </div>
                <em>{isLandscape ? '297x210mm' : '210x297mm'}</em>
              </div>

              <div className="lv-nano-resolution">
                {(Object.keys(A4_RESOLUTIONS) as A4Resolution[]).map(key => (<button key={key} type="button" className={nanoResolution === key ? 'active' : ''} onClick={() => setNanoResolution(key)}>
                    <span>{A4_RESOLUTIONS[key].label}</span>
                    <small>{A4_RESOLUTIONS[key].description}</small>
                  </button>))}
              </div>

              <div className="lv-nano-templates">
                {A4_NANO_TEMPLATE_PROMPTS.map((template, index) => (<button key={template} type="button" onClick={() => {
                setNanoPrompt(template);
                setNanoError(null);
            }}>
                    Template {index + 1}
                  </button>))}
              ...
              <div className="lv-nano-prompt-wrap">
                {nanoReferenceImages.length > 0 && (<div className="lv-nano-reference-list">
                    {nanoReferenceImages.map((image, idx) => (<div key={`${image.name}-${idx}`} className="lv-nano-reference-chip">
                        <img src={image.dataUrl} alt=""/>
                        <span>{image.name}</span>
                        <button type="button" aria-label={`Remove reference image ${idx + 1}`} onClick={() => {
                    setNanoReferenceImages(prev => prev.filter((_, imageIdx) => imageIdx !== idx));
                    if (nanoReferenceInputRef.current)
                        nanoReferenceInputRef.current.value = '';
                }}>
                          x
                        </button>
                      </div>))}
                  </div>)}
                <textarea className="lv-nano-prompt" value={nanoPrompt} maxLength={4000} rows={4} onChange={e => {
            setNanoPrompt(e.target.value);
            setNanoError(null);
        }} onPaste={handleNanoPromptPaste} placeholder="e.g. Saudi Arabia"/>
                <input ref={nanoReferenceInputRef} type="file" accept="image/*" multiple className="lv-nano-reference-input" onChange={e => {
            void handleNanoReferenceUpload(e.target.files || null);
            e.currentTarget.value = '';
        }}/>
                <div className="lv-nano-prompt-actions">
                  <div className="lv-nano-prompt-tools">
                    <button type="button" className="lv-nano-reference-add" title="Add logo or reference images" aria-label="Add logo or reference images" onClick={() => nanoReferenceInputRef.current?.click()}>
                      +
                    </button>
                    <button type="button" className="lv-nano-voice-btn" title={nanoListening ? 'Stop voice input' : 'Speak cover prompt'} aria-label={nanoListening ? 'Stop voice input' : 'Speak cover prompt'} aria-pressed={nanoListening} onClick={toggleNanoVoicePrompt}>
                      {nanoListening ? 'Stop' : 'Mic'}
                    </button>
                    <span>Attach up to 6 images or type prompt - {nanoPrompt.length.toLocaleString()} / 4,000</span>
                  </div>
                  <button type="button" className="lv-nano-generate" disabled={nanoGenerating || !nanoPrompt.trim()} onClick={() => void generateNanoCover()} aria-label="Generate and use cover">
                    {nanoGenerating ? '...' : 'Generate'}
                  </button>
                </div>
              </div>

              <div className="lv-nano-meta">
                <span>{nanoDimensions.width.toLocaleString()} x {nanoDimensions.height.toLocaleString()}px</span>
                <span>{nanoDimensions.dpi} DPI</span>
              </div>

              {nanoImageUrl && (<div className="lv-nano-preview">
                  <img src={nanoImageUrl} alt="Generated A4 cover"/>
                  <button type="button" onClick={() => void applyGeneratedCoverImage(nanoImageUrl)}>
                    Use as cover
                  </button>
                </div>)}

              {nanoError && <div className="lv-nano-error">{nanoError}</div>}
            </div>)}
          </SbSection>
